...
// NewWriterLevelBuffer let the user provide compression level and buffer size values
func NewWriterLevelBuffer2(w io.Writer, level, bufferSize,method, windowBits, memLevel, strategy int) (*Writer, error) {
	z := &Writer{w: w, out: make([]byte, bufferSize)}
	if err := z.strm.deflateInit2(level,method, windowBits, memLevel, strategy); err != nil {
		return nil, err
	}
	return z, nil
}
